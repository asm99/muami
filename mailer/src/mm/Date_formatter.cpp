 ...
 * Date_formatter.cpp - date strings formatting related things
 * Date: 04 August 2015
 */
 ...
 */
std::string
Date_formatter::format_date(std::string s)
{
    struct tm tm;
    strptime(s.c_str(), "%d-%b-%Y %H:%M:%S", &tm);
...
main()
{
    ...
    std::string date("24-Apr-2015 16:05:09");
    std::cout << "Initial date: " << date << std::endl;
    std::cout << date << " => " << df->format_date(date) << std::endl;
    ...
    std::cout << date2 << " => " << df->format_date(date2) << std::endl;

    // Print current date
    std::string cur_date = df->current_date();
    std::cout << "Current date: " << cur_date << std::endl;
